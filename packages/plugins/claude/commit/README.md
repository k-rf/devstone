...
このプラグインは汎用層として動作する。コミットメッセージのスタイルやリポジトリ固有の規約は、プラグイン本体にハードコードせず、**利用側リポジトリの設定ファイルを読み込んで動作を決定する**。
...
リポジトリ固有の規約（課題キー必須化、禁止形式など）は、プラグイン本体（スキーマ・スキル・既定ドキュメント）ではなく、利用側リポジトリの上記設定に記述する。汎用層へリポジトリ特化の規約を書き戻さないこと。

## 設定

プロジェクトルートに `.claude/configs/commit.json` を作成して設定をカスタマイズできる。プラグインはこのファイルを読み込み、メッセージ生成の動作を決定する。
