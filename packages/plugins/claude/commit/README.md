...
コミットメッセージのスタイルやリポジトリ固有の規約は、プラグイン本体にハードコードしない。**利用側リポジトリの設定ファイルを読み込んで動作を決定する**。
...
リポジトリ固有の規約（課題キー必須化、禁止形式など）は、プラグイン本体（スキーマ・スキル・既定ドキュメント）ではなく、利用側リポジトリの上記設定に記述する。プラグイン本体へリポジトリ特化の規約を書き戻さないこと。
