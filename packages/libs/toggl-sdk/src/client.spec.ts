...
import { makeTogglApiClient } from "./client.js";
...
const createdWith = "test-client";
const clientEffect = makeTogglApiClient(apiToken, workspaceId, { createdWith: createdWith });
...
describe("異常系", () => {
  ...
  it("ペイロードのエンコード失敗", async () => {
    ...
    const error = await Effect.runPromise(Effect.flip(program));
    expect(error.message).toBe("Failed to encode TogglTimeEntry");
  });
});
