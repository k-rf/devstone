...
### `PROJECT_ROOT` 環境変数（重要・非自明）

- カバレッジ集約スクリプト（`packages/configs/vitest/src/scripts/*.ts`）は環境変数 `PROJECT_ROOT` を必須で参照します。
- これはワークスペースのルートを指す必要があり、本環境では `PROJECT_ROOT=/workspace` を `~/.bashrc` に設定済みです
  （作者の devcontainer では `/workspaces/devstone`。`.devcontainer/.env` 由来）。
- 未設定だと `pnpm run test:cov` / `pnpm run check:all` が
  `Missing data at PROJECT_ROOT` で失敗します。
...
- 現状のコミット済みツリーでは `knip` ステップが
  未使用ファイル `apps/notion-toggl-bridge/src/utils/promise-chain.ts` を検出して失敗します
  （環境ではなくコード側の既存事象）。`format` / `typecheck` / `lint` / `lint:md` / `test:cov` は通過します。
