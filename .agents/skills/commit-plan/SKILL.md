...
- `style`: コミットメッセージのスタイル（本リポジトリは `gitmoji`）
- `language`: メッセージの言語（`en`、`ja` 等）

設定ファイルが存在しない場合、`gitmoji` スタイル・英語メッセージをデフォルトとする。

課題キー（`DEV-<数字>`）はチケットまたはブランチ名から取得し、すべてのコミットヘッダーに含める。
特定できない場合は計画作成を中止し、ユーザーに確認を求める。Conventional Commits は使わない。
