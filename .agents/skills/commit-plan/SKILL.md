---
name: commit-plan
description: 差分を元にしてコミット計画、コミット分割、コミットの整理、コミットプランの作成を行う際の手順とルール。
---
...
5. 依存関係の順序を決定する
6. プロジェクトの規約（`commit-message` スキルなど）に従ってコミットメッセージを生成する

## 設定

`.agents/configs/commit.json` からプロジェクト固有の設定を読み込む：
...
メッセージの生成規則（スタイルの適用、混在禁止等）は `commit-message` スキルに従う。
`customPrefix` が設定されている場合、チケットや作業ブランチ名からプレフィックスを取得して計画に含める。特定できない場合は計画作成を中止し、ユーザーに確認を求める。
...
```markdown
### Commit N: <prefix> [customPrefix] <件名>
...
**本文**: この変更がなぜ行われたかを説明するコミットメッセージ of body.
