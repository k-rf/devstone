...
`.agents/configs/commit.json` が存在する場合、JSON からコミットスタイルと言語設定を抽出し、`commit-planner` エージェントに渡す。
...
1. 課題キー（`DEV-<数字>`）をチケットまたはブランチ名から確認する。特定できない場合はコミットせずユーザーに確認を求めて停止する
2. ステージ対象のファイルとコミットメッセージを表示する（形式: `<emoji> <課題キー> <件名>`）
3. 特定のパスでファイルをステージする（`git add -A` は使わない）
4. パーシャルステージングが必要な場合：
   - ファイルの最終状態を一時的な場所に保存する
   - HEAD から復元し、このコミットの変更のみを適用する
   - 中間状態をステージする
   - コミット後に最終状態を復元する
5. `package.json` の変更時は `pnpm install` を実行して lockfile を再生成する
6. HEREDOC 形式でコミットを作成する。
   Conventional Commits（`feat:` 等）は使わない：
